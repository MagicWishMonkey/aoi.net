...
namespace AOI.Crypto {
    ...
    public class PBKDF2 {
        public static string Compute(string input, string salt, int iterations) {
            Rfc2898DeriveBytes hasher = new Rfc2898DeriveBytes(input, System.Text.Encoding.Default.GetBytes(salt), iterations);
            var bytes = hasher.GetBytes(256);
